...
const Cryptogramme = ({ onComplete, onNext, letters, gameName, completed }) => {
  const encodedMessage = "WKLV LV D WHVW";
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen w-full bg-[radial-gradient(circle_at_center,rgba(17,24,39,0.9)_0%,rgba(55,65,81,0.9)_100%)] font-mono">      {/* Titre */}
      ...
      <div className="bg-gray-900 border-4 border-gray-800 rounded-xl p-6 w-full max-w-md mx-auto flex flex-col items-center">
        ...
        <div className="text-2xl font-mono bg-gray-800 text-gray-300 p-4 rounded-lg mb-4 inline-block">
          {encodedMessage.split('').map((char, index) => (
            <span
              key={index}
              className={`mx-1 ${mapping[char] && isCorrect(char, mapping[char]) ? 'text-green-500' : 'text-gray-300'}`}
            >
              {mapping[char] || char}
            </span>
          ))}
          ...
          {encodedMessage
            ...
            .map(encodedChar => (
              <div key={encodedChar} className="text-center">
                <div className="text-xl text-gray-300 mb-1">{encodedChar}</div>
                <select
                  className={`p-1 rounded bg-gray-800 text-gray-300 border ${mapping[encodedChar] && isCorrect(encodedChar, mapping[encodedChar]) ? 'border-green-500' : 'border-gray-600'} cursor-pointer text-sm`}
      ...
      <div className="mt-6 flex flex-col items-center mb-6">
        ...
        <button
          onClick={onNext}
          className={`px-6 py-3 bg-gray-900 text-gray-300 border border-gray-800 rounded-lg font-semibold transition-all duration-200 font-mono ${completed ? 'hover:bg-gray-800 hover:text-green-500' : 'opacity-50 cursor-not-allowed'}`}
          disabled={!completed}
        >
          Next
        </button>
